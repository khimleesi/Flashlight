...
{
	FL_LOG("[AUDIO MANAGER CONSTRUCT]", FL_LOG_EMPTY, LOG_BREAK);
}
...
bool AudioManager::Initialize() {
	...
	if (numDrivers == 0)
	{
		...
		FL_LOG("[FMOD AUDIO] No sound drivers found, cannot play sounds", FL_LOG_EMPTY, LOG_WARN);
...
void AudioManager::Shutdown() {
	...
	FL_LOG("[FMOD AUDIO] Size of sounds cache before sounds destroyed: ", soundCount, LOG_RESOURCE);
	
	for (auto& sound : m_sounds) { 
		FL_LOG("[FMOD AUDIO] Destroying sound: ", sound.first.c_str(), LOG_RESOURCE);
		GetErrors(sound.second->release());
		soundCount--;
		if (soundCount == 0) { break; }
	}
	FL_LOG("[FMOD AUDIO] Size of sounds cache after sounds destroyed: ", soundCount, LOG_RESOURCE);

	GetErrors(m_studioSystem->release());
	m_lowLevelSystem	= nullptr;
	m_studioSystem		= nullptr;

	FL_LOG("[AUDIO MANAGER SHUT DOWN]", FL_LOG_EMPTY, LOG_BREAK);
}
...
bool AudioManager::LoadAudio(const std::string& tag, const std::string& soundFile, bool isLooping, float delayCallback, bool isStream, bool is3Dsound)
{
	...
	{
		FL_LOG("[FMOD AUDIO] Sound file already exists, re-using: ", tag.c_str(), LOG_RESOURCE);
		return false;
	}
	...
	if (soundHandle) { 
		...
		FL_LOG("[FMOD AUDIO] New sound loaded successfully: ", src.c_str(), LOG_RESOURCE);
	}
...
void AudioManager::UnloadAudio(const std::string& tag)
{
	...
	if (found != m_sounds.end()) {
		...
		FL_LOG("[FMOD AUDIO] Sound destroyed from memory and removed from sound cache: ", tag.c_str(), LOG_RESOURCE);
	}
	else { FL_LOG("[FMOD AUDIO] Can't unload sound, sound not found: ", tag.c_str(), LOG_RESOURCE); }
}
...
bool AudioManager::PlayAudio(const std::string& tag, float volume, const glm::vec3& position)
{
	...
	if (found == m_sounds.end()) {
		FL_LOG("[FMOD AUDIO] Trying to play a sound that hasn't been loaded: ", tag.c_str(), LOG_ERROR);
		return false;
	}
...
void AudioManager::StopChannel(const std::string& tag)
{
	...
	else { FL_LOG("[FMOD AUDIO] Couldn't find sound to stop channel: ", tag.c_str(), LOG_WARN); }
}
...
void AudioManager::SetChannel3DPosition(const std::string& tag, const glm::vec3& position)
{
	...
	else { FL_LOG("[FMOD AUDIO] Couldn't find sound to set channels 3D position: ", tag.c_str(), LOG_WARN); }
}
...
void AudioManager::SetChannelVolume(const std::string& tag, float volume)
{
	...
	else { FL_LOG("[FMOD AUDIO] Couldn't find sound to set channels volume: ", tag.c_str(), LOG_WARN); }
}
...
void AudioManager::PauseChannel(const std::string& tag, bool paused)
{
	...
	else { FL_LOG("[FMOD AUDIO] Couldn't find sound to set channels pause state: ", tag.c_str(), LOG_WARN); }
}
...
int AudioManager::GetErrors(FMOD_RESULT result) {
	...
	if (result != FMOD_OK) {
		FL_LOG("[FMOD AUDIO] ", FMOD_ErrorString(result), LOG_WARN);
		return 1;
	}
