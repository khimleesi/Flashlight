...
ObjLoader::ObjLoader()
{
	FL_LOG("[OBJ LOADER CONSTRUCT]", FL_LOG_EMPTY, LOG_BREAK);
}
...
ObjLoader::~ObjLoader()
{
	FL_LOG("[OBJ LOADER DESTRUCT]", FL_LOG_EMPTY, LOG_BREAK);
}
...
bool ObjLoader::LoadObjFile(const std::string& obj, std::vector<glm::vec3>& outVertices, std::vector<glm::vec2>& outTextureCoords, std::vector<glm::vec3>& outNormals, std::vector<unsigned int>* outIndices)
{
	...
	if (!File::Instance()->OpenForReading(obj.c_str())) { return false; }
	...
	while (File::Instance()->ExtractFileData()) {
		...
		if		(File::Instance()->FileDataContains(VERTICES))			{ GetVertices(inVertices); }
		else if (File::Instance()->FileDataContains(TEXTURE_COORDS))	{ GetTextureCoords(inTextureCoords); }
		else if (File::Instance()->FileDataContains(NORMALS))			{ GetNormals(inNormals); }
		else if (File::Instance()->FileDataContains(FACES))				{ GetIndices(vertexIndices, textureCoordIndices, normalIndices); }
	}
...
void ObjLoader::GetVertices(std::vector<glm::vec3>& inVertices)
{
	...
	std::string vertices = File::Instance()->CutFileData(file_constants::VERTICES_OFFSET);
...
void ObjLoader::GetTextureCoords(std::vector<glm::vec2>& inTextureCoords)
{
	...
	std::string textureCoords = File::Instance()->CutFileData(file_constants::TEXTURE_COORDS_OFFSET);
...
void ObjLoader::GetNormals(std::vector<glm::vec3>& inNormals)
{
	...
	std::string normals = File::Instance()->CutFileData(file_constants::NORMALS_OFFSET);
...
void ObjLoader::GetIndices(std::vector<unsigned int>& vertexIndices, std::vector<unsigned int>& textureCoordIndices, std::vector<unsigned int>& normalIndices)
{
	...
	std::string faces = File::Instance()->CutFileData(file_constants::FACES_OFFSET);
