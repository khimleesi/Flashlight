...
VertexBuffer::~VertexBuffer()
{
	FL_GLCALL(glDeleteBuffers(1, &m_vertexBufferObject));

	FL_LOG("[VERTEX BUFFER] Vertex buffer object destroyed: ", m_vertexBufferObject, LOG_MEMORY);
}
...
{
	FL_GLCALL(glBindBuffer(GL_ARRAY_BUFFER, m_vertexBufferObject));
}
...
{
	FL_GLCALL(glBindBuffer(GL_ARRAY_BUFFER, 0));
}
...
{
	FL_GLCALL(glDrawArrays(mode, 0, m_vertexCount));
}
...
bool VertexBuffer::Push(const std::vector<PackedVertex>& data, bool dynamic)
{
	...
	if (data.empty()) {
		FL_LOG("[BUFFER] Model vertex data vector container is empty", FL_LOG_EMPTY, LOG_ERROR); return false;
	}
	...
	FL_GLCALL(glBufferData(GL_ARRAY_BUFFER, data.size() * sizeof(PackedVertex), &data.front(), (dynamic)	? GL_DYNAMIC_DRAW
																											: GL_STATIC_DRAW));
...
{
	FL_GLCALL(glVertexAttribPointer(layoutType, s_bufferElements[layoutType], dataType, GL_FALSE, stride, (size_t*)offset));
	FL_GLCALL(glEnableVertexAttribArray(layoutType));
}
...
{
	FL_GLCALL(glGenBuffers(1, &m_vertexBufferObject));

	FL_LOG("[VERTEX BUFFER] Vertex buffer object created: ", m_vertexBufferObject, LOG_MEMORY);
}
