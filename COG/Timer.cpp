...
bool Timer::Start()
{
	...
	if (!m_useChrono)
	{
		...
		{
			FL_LOG("[TIMER] Could not acquire frequency for windows timer", FL_LOG_EMPTY, LOG_ERROR);
			return false;
		}
...
void Timer::Reset()
{
	...
	if (!m_useChrono) { 
		...
		{
			FL_LOG("[TIMER] Could not initialize start time for windows timer", FL_LOG_EMPTY, LOG_ERROR);
		}
	}
...
float Timer::ElapsedSeconds()
{
	...
	if (!m_useChrono)
	{
		...
		{
			FL_LOG("[TIMER] Could not get current time for windows timer", FL_LOG_EMPTY, LOG_ERROR);
			return 0.0f;
		}
