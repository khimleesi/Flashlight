...
ScreenManager::ScreenManager()	:	m_title("SDL OPEN GL Window"),
									...
									m_orthographic(1.0f)
{
	FL_LOG("[SCREEN MANAGER CONSTRUCT]", FL_LOG_EMPTY, LOG_BREAK);
}
...
void ScreenManager::ShutDown()
{
	...
	FL_LOG("[SCREEN MANAGER SHUT DOWN]", FL_LOG_EMPTY, LOG_BREAK);
}
...
{
	if (enableBlending) { FL_GLCALL(glEnable(GL_BLEND)); FL_GLCALL(glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)); }
	else				{ FL_GLCALL(glDisable(GL_BLEND)); }
}
...
{
	if (enableDepth)	{ FL_GLCALL(glEnable(GL_DEPTH_TEST));}
	else				{ FL_GLCALL(glDisable(GL_DEPTH_TEST)); }
}
...
{
	if (enableDepthWriting) { FL_GLCALL(glDepthMask(GL_TRUE)); }
	else					{ FL_GLCALL(glDepthMask(GL_FALSE)); }
}
...
{
	if (cull)	{ FL_GLCALL(glEnable(GL_CULL_FACE)); FL_GLCALL(glCullFace(GL_BACK)); }
	else		{ FL_GLCALL(glDisable(GL_CULL_FACE)); }
}
...
{
	if (wireframe)	{ FL_GLCALL(glPolygonMode(GL_FRONT_AND_BACK, GL_LINE)); }
	else			{ FL_GLCALL(glPolygonMode(GL_FRONT_AND_BACK, GL_FILL)); }
}
...
{	
	FL_GLCALL(glClearColor(r, g, b, 1.0f));
	FL_GLCALL(glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT));
}
...
{
	if (SDL_Init(SDL_INIT_EVERYTHING) == -1)	{ FL_LOG("[SCREEN] SDL failed to initialize", FL_LOG_EMPTY, LOG_ERROR); return false; }
	else										{ FL_LOG("[SCREEN] SDL initialized successfully!", FL_LOG_EMPTY, LOG_SUCCESS); }
	...
	if (!m_window)	{ FL_LOG("[SCREEN] Game window could not be created", FL_LOG_EMPTY, LOG_ERROR); return false; }
	else			{ FL_LOG("[SCREEN] Game window created successfully!", FL_LOG_EMPTY, LOG_SUCCESS); }
...
void ScreenManager::SetOpenGLAttributes() {
	...
	if (m_coreMode)	{ SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_CORE);
					  FL_LOG("[SCREEN] OpenGL CORE mode enabled", FL_LOG_EMPTY, LOG_MESSAGE); }
	else			{ SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_COMPATIBILITY); 
					  FL_LOG("[SCREEN] OpenGL COMPATIBILITY mode enabled", FL_LOG_EMPTY, LOG_MESSAGE); }

	//--- Set OpenGL context version
	SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, m_version); FL_LOG("[SCREEN] OpenGL version in use: ", m_version, LOG_MESSAGE);
	SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, m_subVersion); FL_LOG("[SCREEN] OpenGL sub version in use: ", m_subVersion, LOG_MESSAGE);
}
...
bool ScreenManager::CreateOpenGLContext()
{
	...
	if (!m_context) { FL_LOG("[SCREEN] OpenGL context could not be created", FL_LOG_EMPTY, LOG_ERROR); return false; }
	else			{ FL_LOG("[SCREEN] OpenGL context created successfully!", FL_LOG_EMPTY, LOG_SUCCESS); }

	//--- Check if vSync is enabled or disabled
	if (m_vSync)	{ SDL_GL_SetSwapInterval(1); FL_LOG("[SCREEN] VSync currently enabled", FL_LOG_EMPTY, LOG_MESSAGE); }
	else			{ SDL_GL_SetSwapInterval(0); FL_LOG("[SCREEN] VSync currently disabled", FL_LOG_EMPTY, LOG_MESSAGE); }

	if (SDL_GL_SetSwapInterval(1) == -1) { FL_LOG("[SCREEN] Unable to set VSync", FL_LOG_EMPTY, LOG_ERROR); }
...
bool ScreenManager::DisplayGPUProfile()
{
	...
	if (glewInit() != GLEW_OK) { FL_LOG("[SCREEN] Could not initialize GLEW", FL_LOG_EMPTY, LOG_ERROR); return false; }
	else {
		FL_LOG("[SCREEN] GPU Manufacturer: ", glGetString(GL_VENDOR), LOG_MESSAGE);
		FL_LOG("[SCREEN] GPU Make and Model: ", glGetString(GL_RENDERER), LOG_MESSAGE);
		FL_LOG("[SCREEN] Current OpenGL version and mode: ", glGetString(GL_VERSION), LOG_MESSAGE);
		FL_LOG("[SCREEN] GLSL Language supported: ", glGetString(GL_SHADING_LANGUAGE_VERSION), LOG_MESSAGE);
	}
...
void ScreenManager::DisplayGPUExtensions()
{
	...
	FL_GLCALL(glGetIntegerv(GL_NUM_EXTENSIONS, &numExtensions));

	extensions.reserve(numExtensions);

	for (GLint x = 0; x < numExtensions; x++) { extensions.emplace_back(glGetStringi(GL_EXTENSIONS, x)); }
	
	FL_LOG("[EXTENSIONS] Supported Extensions on GPU:", FL_LOG_EMPTY, LOG_MESSAGE);

	for (GLuint x = 0; x < extensions.size(); x++) { FL_LOG("[EXT] ", extensions[x], LOG_MESSAGE); }
}
...
void ScreenManager::SetupAnisotropicFiltering()
{
	...
	if (glewIsSupported("GL_EXT_texture_filter_anisotropic") || GLEW_EXT_texture_filter_anisotropic)
	{
		...
		FL_GLCALL(glGetFloatv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &maxAnisotropy));
		
		m_anisotropy			= min(desiredAnisotropy, maxAnisotropy);
		
		m_isAnisotropySupported = true;
		
		FL_LOG("[SCREEN] Anisotropic filtering is supported. Hello fancy graphics!", FL_LOG_EMPTY, LOG_SUCCESS);
	}
	else { 
		FL_LOG("[SCREEN] Anisotropic filtering not supported on your graphics card", FL_LOG_EMPTY, LOG_WARN); 
		m_isAnisotropySupported = false;
	}
}
